...
class organisms:
    def __init__(self,group,sp,fen,offs):
        self.group = group
...
class environment:
            ...
            if group == 'A':
                for i in range(self.numSpAn): #loop sobre numero de especies
                    for j in range(self.numInd): #loop sobre o numero de individuos
                        ...
                        self.ind = organisms(group,i,self.fenInitial,self.offspring) #criando um organismo animal
                        self.ind.age = 1 #para que a pop criada seja de adultos
                ...
                for i in range(self.numSpPlan): #loop sobre numero de especies
                    for j in range(self.numInd): #loop sobre o numero de individuos
                        self.fenInitial = self.theta + np.random.uniform()
                        self.ind = organisms(group,i,self.fenInitial,self.offspring) #criando um organismo planta
                        self.ind.age = 1 #para que a pop criada seja de adultos
                        self.population.append(self.ind) #adicionando o novo organismo no final do array de organismos
    ...
    def animalReproduction(self): #reproducao assexuada (depende apenas da interacao)
        ...
        if len(self.animalsPop) <= self.carringCapacity:
...
#                    self.intParGroup = [x for x in self.population if x.group != self.indGroup] #agrupando organismos do outro grupo para interaca
                    ...
                    if self.indIntProb > np.random.uniform():
                        ...
                        if self.indFitness > np.random.uniform():
                            ...
                            self.ind = organisms('A',self.indSp,self.indFen+np.random.normal(0,1),self.offspring) #criando um novo organismo...
    ...
    def plantsReproduction(self): #reproducao assexuada (depende apenas da interacao)
        ...
        if len(self.plantsPop) <= self.carringCapacity:
...
#                    self.intParGroup = [x for x in self.population if x.group != self.indGroup] #agrupando organismos do outro grupo para interaca
                    ...
                    if self.indIntProb > np.random.uniform():
                        ...
                        if self.indFitness > np.random.uniform():
                            ...
                            self.ind = organisms('P',self.indSp,self.indFen+np.random.normal(0,1),self.offspring) #criando um novo organismo...
    ...
    def death(self):
        ...
        self.population = [x for x in self.population if x.statusLife == 1] #eliminando os mortos
