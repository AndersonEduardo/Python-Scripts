...
df = pd.read_csv('../data/example.csv', index_col='UID',
                 names=['UID', 'First Name', 'Last Name', 'Age', 'Pre-Test Score', 'Post-Test Score'])
df

# Load a csv while setting the index columns to First Name and Last Name
df = pd.read_csv('../data/example.csv', index_col=['First Name', 'Last Name'],
                 names=['UID', 'First Name', 'Last Name', 'Age', 'Pre-Test Score', 'Post-Test Score'])
df
...
# slicing by row and columns indexes
df.iloc[0:1,:]
df.iloc[2:4,[0,3]]

# subsetting using criteria
df[df.age >= 50]
df[(df.age >= 50) & (df.postTestScore < 90)]